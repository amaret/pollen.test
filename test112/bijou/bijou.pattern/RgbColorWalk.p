...
module RgbColorWalk {
  ...
  public start(Scene s) {
    ...
    print "Starting RgbColorWalk timer, numTicks=" + numTicks + "\n"
  ...
  draw() {    
    uint16 v = Lookup.map(tickCount, 0, numTicks, 0, patternLength)
    uint8 state = v / 256
...
//    print "Drawing, tickCount=" + tickCount + " v=" + v + " val= " + val + " (rgb)=(" + red + "," + green + "," + blue + ")\n"
    ledState.setRGB(red, green, blue).apply()
