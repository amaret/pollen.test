...
from pollen.hardware_old import Uart as UartProtocol
...
# Calculate initial values for default baud rate
        baudh = (((Mcu.host_getFrequency() / (baudRate * 16)) - 1) >> 8)
        baudl = (((Mcu.host_getFrequency() / (baudRate * 16)) - 1))
    }
...
public flush() {
    ...
    public setBaud(uint32 baud) {
        baudRate = baud
        + {UBRR0H}+ = (((Mcu.getFrequency() / (baudRate * 16)) - 1) >> 8)
        + {UBRR0L}+ = (((Mcu.getFrequency() / (baudRate * 16)) - 1))
    }

    public host setBaud(uint32 baud) {
        baudRate = baud
        baudh = (((Mcu.getFrequency() / (baudRate * 16)) - 1) >> 8)
        baudl = (((Mcu.getFrequency() / (baudRate * 16)) - 1))
    }
