...
class Scene {
  ...
  host PatternProtocol.stop() stopP = null                  // this should be switched to an event later.   
  host PatternProtocol.stopped() stoppedP = null           // this should be switched to an event later. 

  host uint32 duration = 0

  public host Scene(uint32 duration, PatternProtocol.start shandler, PatternProtocol.stop sthandler) {
    @duration = duration
    @startP = shandler
    @stopP = sthandler
  }
  ...
  public setDuration(uint32 duration) {
    @duration = duration
  }
