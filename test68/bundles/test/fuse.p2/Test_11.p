...
module Test_11 {
  ...
  const uint8 PWMDEFAULT = 0x19  
  const uint8 PWMMAX = 255      // limit the brightness here. 
  ...
  doFadeState(uint8 state) {    
    ...
    if (state == FadeState.PULSEUP) {
      ...
      min = rgb.getMinValue()
      fadeInStep = fadeInMs / (PWMMAX - min) 
      fadeOutStep = fadeOutMs / (PWMMAX - min)
      fadeTickCount = 0
  ...
  updateFadeToWhite() {
    if (fadeState == FadeState.WHITE || fadeState == FadeState.PULSEUP) {
      if (compbuff[RED] < PWMMAX) { compbuff[RED] += 1 }
      if (compbuff[GREEN] < PWMMAX) { compbuff[GREEN] += 1 }
      if (compbuff[BLUE] < PWMMAX) { compbuff[BLUE] += 1 }
    }
  }
